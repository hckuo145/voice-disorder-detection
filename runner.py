...
class Runner():
    ...
    def _train_step(self, source_batch, target_batch, mode='naive'):
        ...
        self.metrics['train/source/cls_loss'] += source_cls_loss.item()
        self.metrics['train/source/dmn_loss'] += source_dmn_loss.item()
        self.metrics['train/target/cls_loss'] += target_cls_loss.item()
        self.metrics['train/target/dmn_loss'] += target_dmn_loss.item()
    ...
    def _valid_step(self, source_batch, target_batch, mode='naive'):
        ...
        self.metrics['valid/source/cls_loss'] += source_cls_loss.item() * len(source_x)
        self.metrics['valid/source/dmn_loss'] += source_dmn_loss.item() * len(source_x)
        self.metrics['valid/target/cls_loss'] += target_cls_loss.item() * len(source_x)
        self.metrics['valid/target/dmn_loss'] += target_dmn_loss.item() * len(source_x)
    ...
    def train(self):
        while self.epoch < self.max_epoch:
            ...
            for source_batch, target_batch in tqdm(self.loader['train']):
            ...
            for source_batch, target_batch in tqdm(self.loader['valid']):
                source_batch = list(map(lambda item: item.to(self.device), source_batch))
                target_batch = list(map(lambda item: item.to(self.device), target_batch))
            ...
            self.metrics['valid/source/nor'] = source_recalls[0]
            self.metrics['valid/source/neo'] = source_recalls[1]
            self.metrics['valid/source/pho'] = source_recalls[2]
            self.metrics['valid/source/uar'] = np.mean(source_recalls)
            self.metrics['valid/target/nor'] = target_recalls[0]
            self.metrics['valid/target/neo'] = target_recalls[1]
            self.metrics['valid/target/pho'] = target_recalls[2]
            self.metrics['valid/target/uar'] = np.mean(target_recalls)
    ...
    def test(self, checkpoint=None):
        ...
        self.metrics['test/source/nor'] = source_recalls[0]
        self.metrics['test/source/neo'] = source_recalls[1]
        self.metrics['test/source/pho'] = source_recalls[2]
        self.metrics['test/source/uar'] = np.mean(source_recalls)
        self.metrics['test/target/nor'] = target_recalls[0]
        self.metrics['test/target/neo'] = target_recalls[1]
        self.metrics['test/target/pho'] = target_recalls[2]
        self.metrics['test/target/uar'] = np.mean(target_recalls)
